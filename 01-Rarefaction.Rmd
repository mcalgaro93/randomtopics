...
For this reason we use the [`subsampleCounts`](https://rdrr.io/github/FelixErnst/mia/man/subsampleCounts.html) function of the package `mia` to obtain subsampled counts. 
We repeat this process 3 times with 3 different seeds (1, 2, and 3). 
We store the results in three differentially named assays of the `TreeSummarizedExperiment` object: *rare1*, *rare2*, and *rare3*.
...
An alternative is to check our methods, and redo the taxonomy classification. 
Try loading `taxonomy-dada.txt` and you will now see a different story...